...
"""
SAM3 SageMaker Training Script

This script launches SAM3 fine-tuning jobs on AWS SageMaker using the remote function decorator.
It downloads data from S3, runs the training, and uploads results back to S3.

Usage:
    python sam3/train/train_sam3_sagemaker.py --config sam3/train/configs/solar_anomaly/solar_anomaly_finetune_sagemaker.yaml
"""
...
# Configuration
# ============================================================================
PROJECT_NAME = "sam3-solar-anomaly-detection"

# API Keys
WANDB_API_KEY = os.environ.get("WANDB_API_KEY", "")
WANDB_PROJECT = os.environ.get("WANDB_PROJECT", "sam3-solar-anomaly-detection")
...
MAX_RUNTIME_SECONDS = int(os.environ.get("MAX_RUNTIME_SECONDS", 48 * 60 * 60))  # Default to 48 hours
...
def main(config_content: str, config_name: str) -> dict:
    """
    Main training function that runs on SageMaker.
    ...
    logger.info("=" * 80)
    logger.info("SAM3 SAGEMAKER TRAINING JOB")
    logger.info("=" * 80)
    ...
    if WANDB_API_KEY and wandb_config.get("enabled", True):
        try:
            import wandb
            
            wandb_project = wandb_config.get("project", WANDB_PROJECT)
            wandb_entity = wandb_config.get("entity", WANDB_ENTITY)
            wandb_run_name = wandb_config.get("run_name", f"{config_name}_{INSTANCE_TYPE}")
            ...
            wandb_run = wandb.init(
                ...
                config={
                    ...
                    "instance_type": INSTANCE_TYPE,
                    "gpu_count": INSTANCE_ATTRS.get(INSTANCE_TYPE, {}).get("gpu_count", 1),
                    "gpu_type": INSTANCE_ATTRS.get(INSTANCE_TYPE, {}).get("gpu_type", "unknown"),
                    **config.get("scratch", {}),
                },
                tags=wandb_config.get("tags", ["sam3", "sagemaker", "solar-anomaly"]),
                notes=wandb_config.get("notes", f"SAM3 training on SageMaker {INSTANCE_TYPE}"),
            )
    ...
    # Save the config to a predictable directory for Hydra
...
'''
    ...
    # Ensure __init__.py files exist in all config directories for Hydra's pkg:// resource loading
    # Hydra uses importlib.resources which requires __init__.py in Python < 3.11
    ...
    logger.info("Starting SAM3 Training...")
    ...
    # Log final metrics to WandB and finish run
    if wandb_run is not None:
        try:
            ...
            if checkpoint_dir.exists():
                best_checkpoints = list(checkpoint_dir.glob("best_*.pth")) + list(checkpoint_dir.glob("*best*.pth"))
                if best_checkpoints:
                    best_ckpt = best_checkpoints[-1]  # Get the latest best checkpoint
                    ...
                    wandb_run.log_artifact(artifact, aliases=["latest_best"])
                    logger.info(f"Logged best checkpoint to WandB: {best_ckpt}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Launch SAM3 training job on AWS SageMaker"
    )
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to SAM3 YAML config file (e.g., sam3/train/configs/solar_anomaly/solar_anomaly_finetune_sagemaker.yaml)"
    )
    parser.add_argument(
        "--config-name",
        type=str,
    ...
    if args.config_name is None:
        # Convert path like "sam3/train/configs/solar_anomaly/solar_anomaly_finetune_sagemaker.yaml"
        # to "solar_anomaly/solar_anomaly_finetune_sagemaker" (relative to configs directory)
        ...
        if "configs" in config_path.parts:
            ...
            # config_name = "/".join(config_path.parts[configs_idx:])
    ...
    print(f"Launching SAM3 SageMaker training job with config: {args.config}")
...
Accept the SAM3 model license at: https://huggingface.co/facebook/sam3 (you need to agree to the license before you can download the weights)
The token is passed securely via environment variables to the SageMaker container


# Set your API keys
export HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxxx  # Get from https://huggingface.co/settings/tokens

# Run training
python sam3/train/train_sam3_sagemaker.py \
    --config sam3/train/configs/solar_anomaly/solar_anomaly_finetune_sagemaker.yaml \
    --config-name configs/solar_anomaly/solar_anomaly_finetune_sagemaker
"""