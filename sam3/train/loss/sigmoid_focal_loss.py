...
# Try to import triton, but make it optional for non-CUDA systems (e.g., Mac)
# Also check GPU compute capability - bf16 requires sm_80+
try:
    import triton
    ...
    # Check if GPU supports bf16 (requires sm_80 or higher)
    # GPUs like T4 (sm_75) don't support bf16 and will fail with Triton kernels
    if torch.cuda.is_available():
        device_cap = torch.cuda.get_device_capability()
        # sm_80 = (8, 0), we need major >= 8 for bf16 support
        if device_cap[0] < 8:
            print(f"WARNING: GPU compute capability {device_cap[0]}.{device_cap[1]} (sm_{device_cap[0]}{device_cap[1]}) "
                  f"does not support bf16. Falling back to PyTorch implementation for sigmoid focal loss.")
...
# PyTorch fallback implementations for non-CUDA systems
def _pytorch_sigmoid_focal_loss(inputs, targets, alpha=0.25, gamma=2):
    """PyTorch implementation of sigmoid focal loss (non-reduced)."""
    prob = inputs.sigmoid()
    ce_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction="none")
    p_t = prob * targets + (1 - prob) * (1 - targets)
    alpha_t = alpha * targets + (1 - alpha) * (1 - targets)
...
def _pytorch_sigmoid_focal_loss_reduce(inputs, targets, alpha=0.25, gamma=2):
    """PyTorch implementation of sigmoid focal loss (reduced)."""
    loss = _pytorch_sigmoid_focal_loss(inputs, targets, alpha, gamma)
    return loss.sum()
...
if TRITON_AVAILABLE:
    ...
    # Use triton implementations
    triton_sigmoid_focal_loss = SigmoidFocalLoss.apply
    triton_sigmoid_focal_loss_reduce = SigmoidFocalLossReduced.apply

else:
    # Use PyTorch fallbacks on non-CUDA systems (e.g., Mac)
    triton_sigmoid_focal_loss = _pytorch_sigmoid_focal_loss
    triton_sigmoid_focal_loss_reduce = _pytorch_sigmoid_focal_loss_reduce
